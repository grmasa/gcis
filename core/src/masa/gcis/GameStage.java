...
import masa.gcis.sky.Cloud;
import masa.gcis.sky.Fan;
import masa.gcis.sky.SkyActor;
...
public class GameStage extends Stage {
   ...
   // List<PhysicsActor> obstacleList = new ArrayList<>();
    ...
    private String[] enemyTypes = {"turkey", "macedonia", "albania"};
    ...
    public GameStage(Viewport viewport, SpriteBatch batch, FollowingCamera camera , Camera b2dCamera, Stage hud) {
        ...
        cannon = new Cannon();
        player = new Greeceball(world, new Vector2(Game.WIDTH/2f-60f, 40f));
        ...
        for(int i=0;i<2;i++){//powerups
            ...
            someBonus.setBeginContactAction(new ActorAction<PhysicsActor, Greeceball>() {
                @Override
                public void commenceOperation(PhysicsActor me, Greeceball him) {
            ...
            someBonus.setEndContactAction(new ActorAction<PhysicsActor, Greeceball>() {
                @Override
                public void commenceOperation(PhysicsActor me, Greeceball him) {
        ...
        for(int i=0;i<2;i++){//enemies
            PhysicsActor someBonus = new PhysicsActor(world, new Vector2((float)(Math.random()*Game.WIDTH/4.0+Game.WIDTH/2.0), (float)(Math.random()*Game.HEIGHT*4.0+Game.HEIGHT*1.5)), "enemy_turkey", BodyDef.BodyType.KinematicBody, "enemy_turkey", true, 50f, true);
            someBonus.setBeginContactAction(new ActorAction<PhysicsActor, Greeceball>() {
                @Override
                public void commenceOperation(PhysicsActor me, Greeceball him) {
                    him.kill();
        ...
        for(PhysicsActor element: thingsToMoveUp){
            ...
            if(element.label.matches("enemy.*")){
				if(player.getScore()>70){
					...
					if(element.label.equals("enemy_macedonia")){
