...
public class MyContactListener implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        if(bodyb.getUserData()!=null && bodya.getUserData()!=null){
            ...
            Greeceball player = null;
            ObstacleActor bottom = null;

            if(bodya.getUserData().getClass() == Greeceball.class
                    && bodyb.getUserData() instanceof  ObstacleActor) {
                player = (Greeceball) bodya.getUserData();
                bottom = (ObstacleActor) bodyb.getUserData();
            } else if(bodyb.getUserData().getClass() == Greeceball.class
                    && bodya.getUserData() instanceof  ObstacleActor) {
                player = (Greeceball) bodyb.getUserData();
                bottom = (ObstacleActor) bodya.getUserData();
            }

            if(bodyb.getUserData() instanceof PhysicsActor && bodya.getUserData() instanceof Greeceball){
                //Gdx.app.debug("Phys", "end contact body: "+physicsActor);
                ((PhysicsActor) bodyb.getUserData()).reactToBeginContact((PhysicsActor)(bodyb.getUserData()), (Greeceball)(bodya.getUserData()) );
            }else if(bodya.getUserData() instanceof PhysicsActor && bodyb.getUserData() instanceof Greeceball){
                //Gdx.app.debug("Phys", "end contact body: "+physicsActor);
                ((PhysicsActor) bodya.getUserData()).reactToBeginContact((PhysicsActor)(bodya.getUserData()), (Greeceball)(bodyb.getUserData()) );
            }
    ...
    public void endContact(Contact contact) {
        ...
        if(bodyb.getUserData()!=null && bodya.getUserData()!=null){
            ...
            if(bodyb.getUserData() instanceof PhysicsActor && bodya.getUserData() instanceof Greeceball){
                //Gdx.app.debug("Phys", "end contact body: "+physicsActor);
                ((PhysicsActor) bodyb.getUserData()).reactToEndContact((PhysicsActor)(bodyb.getUserData()), (Greeceball)(bodya.getUserData()) );
            }else if(bodya.getUserData() instanceof PhysicsActor && bodyb.getUserData() instanceof Greeceball){
                //Gdx.app.debug("Phys", "end contact body: "+physicsActor);
                ((PhysicsActor) bodya.getUserData()).reactToEndContact((PhysicsActor)(bodya.getUserData()), (Greeceball)(bodyb.getUserData()) );
            }

            if((bodya.getUserData() instanceof Greeceball && bodyb.getUserData() instanceof TrampolineActor)
                    ||
                    ( bodyb.getUserData() instanceof Greeceball && bodya.getUserData() instanceof TrampolineActor)) {
